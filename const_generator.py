...
def gen(include_dir: str, output_dir: str):
    ...
    for target in INCLUDE_FILES:
        ...
        outfile.write((templ['header']).encode("utf-8"))
        if target == 'unicorn.h':
            prefix = ''
...
def parse_arguments():
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                description=f"""
{sys.argv[0]} generates dotnet constant files from unicorn C sources.
If one of the options are not submitted, the program will fallback to legacy values:
  ...
  -o ./go/unicorn/
    ...
    parser.add_argument("-o", "--output", default="./UnicornEngine/Const/",  help="Output directory")
